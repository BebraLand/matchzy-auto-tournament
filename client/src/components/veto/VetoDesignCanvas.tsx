...
export type VetoMapStateStyle = 'outline' | 'tint';
...
export type VetoDesign = { version: 1; screens: Record<VetoScreen, VetoLayout>; teamFallback?: VetoTeamFallback; mapStateStyle?: VetoMapStateStyle; mapStateTintOpacity?: number; showMapSideBadges?: boolean; showTimelineOwnership?: boolean; showActionOwnership?: boolean };
...
export function defaultVetoDesign(): VetoDesign {
  ...
  return { version: 1, teamFallback: defaultTeamFallback, mapStateStyle: 'outline', mapStateTintOpacity: 0.14, showMapSideBadges: false, showTimelineOwnership: false, screens: {
...
function TeamLogoFallback({ name, fallback, map = false, mapSize }: { name: string; fallback: VetoTeamFallback; map?: boolean; mapSize?: number }) {
  const text = teamFallbackText(name, fallback);
  if (!text) return null;
  return <span className={map ? 'vdc-map-logo vdc-map-logo-fallback' : 'vdc-empty-image vdc-logo-placeholder'} style={{ background: fallback.background, color: fallback.color, borderColor: fallback.borderColor, borderRadius: fallback.radius, ...(mapSize ? { width: mapSize, height: mapSize } : {}) }}>{text}</span>;
}

function MapPool({ element, veto, maps, logos, fallback, showMapSideBadges, mapStateStyle, mapStateTintOpacity }: { element: VetoElement; veto: VetoState | null; maps: VetoMapMetadata; logos: { team1: string | null; team2: string | null }; fallback: VetoTeamFallback; showMapSideBadges: boolean; mapStateStyle: VetoMapStateStyle; mapStateTintOpacity: number }) {
  ...
  const columns = element.columns || 7;
  const gap = element.gap ?? 12;
  const mapLogoSize = Math.max(1, ((element.w - gap * (columns - 1)) / columns - 4) * 0.44);
  return <div className="vdc-maps" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`, gap }}>
    {names.map((name) => {
      ...
      const tintState = mapStateStyle === 'tint' && stage !== 'available';
      return <div className={`vdc-map ${stage} ${tintState ? 'vdc-map-tinted' : ''} ${hasMapTag ? 'vdc-map-with-tag' : ''}`} key={name} data-stage={stage} data-testid={`broadcast-veto-map-${name}`} style={{ background: element.background || '#0F1B29', borderColor: element.borderColor || '#536A83', borderRadius: element.radius ?? 16 }}>
        ...
        {tintState && <div className="vdc-map-state-tint" aria-hidden="true" style={{ opacity: mapStateTintOpacity }} />}
        <strong className="vdc-map-name">{maps.get(name)?.displayName || getMapDisplayName(name)}</strong>
        {logo ? <img className="vdc-map-logo" src={logo} alt="" /> : pickedTeam && <TeamLogoFallback name={pickedTeamName} fallback={fallback} map mapSize={mapLogoSize} />}
...
export function VetoDesignCanvas({ design, screen, veto, branding, tournamentName = 'Tournament', maps = new Map(), logos = { team1: null, team2: null }, selectedId, guides, distances, onElementPointerDown }: {
  ...
  const mapStateStyle = design.mapStateStyle ?? 'outline';
  const mapStateTintOpacity = Math.max(0, Math.min(0.4, design.mapStateTintOpacity ?? 0.14));
    ...
    <div className="vdc-canvas" data-vdc-screen={screen} style={{ width: 1920, height: 1080, transform: `translate(-50%, -50%) scale(${scale})`, background: layout.background, backgroundImage: layout.backgroundImage ? `url("${layout.backgroundImage}")` : undefined }}>
      ...
      {layout.elements.filter((element) => element.visible !== false).map((element) => {
        ...
        return <div key={renderKey} data-vdc-id={element.id} data-vdc-binding={element.binding || undefined} className={`vdc-element vdc-${element.kind} ${activeTeam ? 'vdc-team-active' : ''} ${selectedId === element.id ? 'selected' : ''} ${onElementPointerDown ? 'editable' : ''}`} style={style} onPointerDown={onElementPointerDown ? (event) => onElementPointerDown(element.id, event, 'move') : undefined}>
          ...
          {element.kind === 'maps' && <MapPool element={element} veto={veto} maps={maps} logos={logos} fallback={fallback} showMapSideBadges={showMapSideBadges} mapStateStyle={mapStateStyle} mapStateTintOpacity={mapStateTintOpacity} />}
