...
test.describe('Tournament match rulings', () => {
  test('records a technical win through bracket progression and removes a voided no-show from execution', async ({
    ...
    try {
      ...
      // A no-winner void is not a tournament champion. Even if this is the
      // final remaining match, the operator must retain the option to reopen
      // it or explicitly close the tournament without a winner later.
      const tournamentAfterVoid = await request.get('/api/tournament');
      expect(tournamentAfterVoid.ok(), await tournamentAfterVoid.text()).toBeTruthy();
      expect((await tournamentAfterVoid.json()).tournament.status).toBe('in_progress');
    } finally {
      await request.delete('/api/tournament');
  ...
  test('does not complete a one-match tournament after a no-winner void', async ({ request }) => {
    const stamp = Date.now();
    const teamIds = [`void-${stamp}-1`, `void-${stamp}-2`];
    ...
    try {
      for (const [index, id] of teamIds.entries()) {
        const createTeam = await request.post('/api/teams', {
          data: {
            id,
            name: `Void Team ${index + 1}`,
            players: Array.from({ length: 5 }, (_, playerIndex) => ({
              steamId: `7656118${String(stamp + index * 10 + playerIndex).slice(-10).padStart(10, '0')}`,
              name: `Void ${index + 1}-${playerIndex + 1}`,
            })),
          },
        });
        expect(createTeam.ok(), await createTeam.text()).toBeTruthy();
      ...
      expect(matches.ok(), await matches.text()).toBeTruthy();
      const onlyMatch = (await matches.json()).matches.find(
        (match: { round: number }) => match.round === 1
      ) as { slug: string };
      expect(onlyMatch?.slug).toBeTruthy();

      const voidNoShow = await request.post(`/api/matches/${onlyMatch.slug}/ruling`, {
        data: { kind: 'void', reason: 'Neither team attended the final' },
      });
      expect(voidNoShow.ok(), await voidNoShow.text()).toBeTruthy();
